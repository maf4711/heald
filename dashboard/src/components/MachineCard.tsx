...
function healthColor(machine: Machine): string {
  ...
  if (machine.disk.smart.some((s) => s.status === "Failing")) return "#ef4444";
  if (machine.ram.pressureLevel >= 4) return "#ef4444";
  ...
  if (machine.ram.pressureLevel >= 2) return "#eab308";
  if (machine.ram.swapUsedMB > 2048) return "#eab308";
  ...
  return (
    <div style={{ background: "#1a1a1a", borderRadius: 12, padding: 20, border: `1px solid ${color}33` }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
        <MetricBox label="CPU" value={`${(machine.cpu.overall * 100).toFixed(1)}%`} />
        <MetricBox label="RAM" value={`${machine.ram.usedGB.toFixed(1)} GB`} sub={`swap: ${machine.ram.swapUsedMB.toFixed(0)} MB`} />
      ...
      <div style={{ fontSize: 12, color: "#999" }}>
        ...
        {machine.processes.topCPU.slice(0, 3).map((p) => `${p.name} ${p.cpuPercent.toFixed(1)}%`).join(", ") || "—"}
      </div>
    </div>
  );
}
